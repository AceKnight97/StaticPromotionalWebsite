...
import { toggleArr, isTheSameObj } from '../../Ultis';
...
const SideBar = (props) => {
  ...
  const onTitleClick = ({ key }) => {
    ...
    if (isTheSameObj(newArr, openArr)) return;
  ...
  useEffect(() => {
    ...
    if (pathname.length > 3) pathname.length = 3;
      ...
      <Menu mode="inline" defaultOpenKeys={[root]} selectedKeys={[state.pathname]}>
        {_.map(menu, item => (item.sub ? (
          <SubMenu
            ...
            title={(
              <div className={classnames('fr-sb', 'pos-re')}>
                <div>
                  {getIcon(item.name)}
                  <span className="side-bar-item-text">{item.name}</span>
                </div>
                <div className="side-bar-item-caret">
                  {openArr.includes(item.link) ? <CaretDownOutlined /> : <CaretUpOutlined />}
                </div>
              </div>
              )}
          >
